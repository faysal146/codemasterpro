...
const Navigation = function () {
    ...
    return (
        <nav
            className="container relative mx-auto px-2 bg-gray-800 rounded justify-between flex my-3 font-bold text-emerald-300 lg:text-lg"
        >
            {
                !isLargeScreen && <Modal isOpen={showSearch}>
                    <div className="fixed top-0 left-0 w-screen h-screen bg-gray-800 opacity-70 z-40" onClick={toggleSearch(false)} />
            ...
            <ul className="flex py-2 relative w-full items-center">
                ...
                }
                {
                    categories.moreCategories.length ? (
                        <div className="relative">
                            <button type="button" onClick={() => setShowMoreNavItem(prev => !prev)} className="py-2 px-2 md:px-3 inline-flex hover:bg-gray-900 rounded font-bold items-center">
                                <span className="mr-1">More</span>
                                {
                                    showMoreNavItem ? <span>
                            ...
                            <div className="absolute flex flex-col space-y-4 bg-gray-800 shadow-md mt-3 p-4 rounded z-20 w-44 right-0" style={{ display: showMoreNavItem ? 'flex' : 'none' }}>
                                {
                ...
                }
                {
                    !isLargeScreen &&
                    (
                        <button type="button" onClick={toggleSearch(true)} className="block rounded px-2 self-stretch text-white bg-emerald-500 ml-auto">
                            <svg
